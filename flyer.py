...
def draw_header(c):
    ...
    txt(c, 'Every booking funds college tuition', W/2, ly - 60,
...
def draw_college_strip(c, y):
    ...
    txt(c, 'Every booking directly funds college tuition.',
        MARGIN + CARD_PAD, by + COLLEGE_H - 17, 'Helvetica-Bold', 9.5, CHARCOAL)
    txt(c, 'You get a spotless car — and a local Plainfield student gets closer to their degree. Thank you for supporting us.',
        MARGIN + CARD_PAD, by + COLLEGE_H - 31, 'Helvetica', 7.5, MUTED)
    txt(c, 'Book local. Support a student. Drive away clean.',
...
def draw_back_page(c):
    ...
    txt(c, 'Run by a local student.', MARGIN, mis_y + 2, 'Helvetica-Bold', 8.5, HexColor('#eeeeee'))
    txt(c, 'Every detail funds college tuition.', MARGIN, mis_y - 12, 'Helvetica', 8, HexColor('#aaaaaa'))
